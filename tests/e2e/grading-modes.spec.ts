...
async function saveAndRelease(page: Page) {
  ...
  await page
    ...
    .click();
  await page
    .getByRole("button", { name: "Confirm release to learner", exact: true })
    .click();
  await expect(
...
test("attempts retain frozen point setups, corrections, and scoped excusals", async ({
  ...
  const cookies = new Map<typeof staff, string>();
    const call = async (
      context: typeof staff,
      path: string,
      data: unknown,
    ) => {
      const response = await context.request.post(`${baseURL}/api${path}`, {
        headers: cookies.has(context)
          ? { Cookie: cookies.get(context)! }
          : {},
        data,
      });
    ...
    const assignmentGradeCards = learnerGrades
      .locator('[data-slot="card"]')
      .filter({
        has: learnerGrades.locator(
          `a[href="/assignments/${assignment}"]`,
        ),
      });
    const staffPage = await staff.newPage();
    await staffPage.goto(
      `${baseURL}/staff/assignments/${assignment}#attempt-${first.submission}`,
    );
    await staffPage
      .getByRole("button", { name: "Assess this attempt", exact: true })
      .click();
    await staffPage
    ...
    await expect(
      assignmentGradeCards.getByText("7 / 10", { exact: true }),
    ).toHaveCount(0);
    await staffPage
      .getByRole("button", { name: "Review release", exact: true })
      .click();
    ...
    await staffPage
      .getByRole("button", { name: "Retract to correct", exact: true })
      .click();
    ...
    await staffPage
      .getByRole("textbox", { name: "Criterion", exact: true })
      .fill("Analysis");
    await staffPage
      .getByRole("spinbutton", { name: "Maximum points", exact: true })
      .fill("12");
    await staffPage
    ...
    await expect(
      setupDialog.getByText(/Existing drafts, released grades, excusals/),
    ).toBeVisible();
    await setupDialog
    ...
    await learnerGrades.reload();
    await expect(
      assignmentGradeCards.getByText("8 / 10", { exact: true }).first(),
    ).toBeVisible();
    await assignmentGradeCards
      .first()
      .getByText("Correction history", { exact: true })
      .click();
    await expect(
      assignmentGradeCards.getByText("7 / 10", { exact: true }).first(),
    ).toBeVisible();
    ...
    await expect(
      assignmentPage.getByText(
        "The latest submitted attempt has not been assessed yet.",
      ),
    ).toBeVisible();
    await expect(
      assignmentPage.getByRole("heading", { name: "Previous assessments" }),
    ).toBeVisible();
    await expect(
      assignmentPage.getByText("8 / 10", { exact: true }).first(),
    ).toBeVisible();

    await staffPage.goto(
      `${baseURL}/staff/assignments/${assignment}#attempt-${second.submission}`,
    );
    await staffPage
      .getByRole("button", { name: "Assess this attempt", exact: true })
      .click();
    await staffPage
      ...
      .click();
    await staffPage
      .getByRole("spinbutton", { name: "Score / 12" })
      .fill("10");
    await staffPage
      .getByRole("spinbutton", { name: "Score / 8" })
      .fill("7");
    ...
    expect(secondAssessment.setupRevision).toBeGreaterThan(
      firstAssessment.setupRevision,
    );
    expect(secondAssessment.outOf).toBe(20);
    expect(
      secondAssessment.criteria.map(
        (criterion: { name: string }) => criterion.name,
      ),
    ).toEqual(["Analysis", "Clarity"]);

    await assignmentPage.reload();
    await expect(
      assignmentPage.getByText("17 / 20", { exact: true }).first(),
    ).toBeVisible();
    await expect(
      assignmentPage.getByText("8 / 10", { exact: true }).first(),
    ).toBeVisible();
    ...
    await staffPage
      .getByRole("option", { name: "Assignment excusal (no attempt)" })
      .click();
    await staffPage
      .getByRole("button", { name: "Record assignment excusal" })
      .click();
    await staffPage
      .getByRole("button", { name: "Excuse assignment", exact: true })
      .click();
    await staffPage
      .getByRole("button", { name: "Confirm excusal to learner" })
      .click();
    await assignmentPage.reload();
    await expect(
      assignmentPage.getByText("17 / 20", { exact: true }).first(),
    ...
    await staffPage.goto(
      `${baseURL}/staff/assignments/${assignment}#attempt-${third.submission}`,
    );
    await staffPage
      .getByRole("button", { name: "Assess this attempt", exact: true })
      .click();
    await staffPage
      ...
      .click();
    await staffPage
      .getByRole("button", { name: "Excuse this assessment", exact: true })
      .click();
    await staffPage
      .getByRole("button", { name: "Confirm excusal to learner" })
      .click();
    await learnerGrades.reload();
    await expect(
      assignmentGradeCards.getByText("Assignment excused."),
    ).toBeVisible();
  await expect(
    assignmentGradeCards.getByText("Attempt 3 excused."),
  ).toBeVisible();
});
