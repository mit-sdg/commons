...
describe("application-owned design integration", () => {
  test("every authored concept is registered through the concept-set module", () => {
    ...
    for (const concept of designs) {
      ...
      expect(conceptSet, concept).toMatch(new RegExp(`^  \\w+: ${directory},$`, "m"));
  ...
  test("the application design records the manually audited external-role bindings", () => {
    const expectedBindings = [
      ...
      "Categorizing.Scope is ForumScope",
      "PickLinking.Source is Publishing.Edition",
      "PickLinking.Target is Piling.Category",
      "Piling.Item is LiveCard",
      "Piling.Scope is Publishing.Edition",
      "Relaying.Author is Authenticating.User",
      "Relaying.Material is Questioning.Questionnaire",
      "RoundInsisting.Aim is LiveSubject",
      "RoundLinking.Source is Publishing.Edition",
      "RoundLinking.Target is Publishing.Edition",
      "RoundReasoning.Reasoner is LiveReasoner",
      "RoundReasoning.Subject is LiveSubject",
      ...
      "Seating.Person is LiveParticipant",
      "Seating.Target is Publishing.Edition",
