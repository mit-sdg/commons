...
describe("the engine package boundary", () => {
  ...
  test("matching beta.14 packages and public commands are pinned", () => {
    const manifest = JSON.parse(readFileSync(join(root, "package.json"), "utf8"));
    expect(manifest.dependencies["@mit-sdg/sync-engine"]).toBe("1.0.0-beta.14");
    expect(manifest.dependencies["@mit-sdg/sync-engine-http"]).toBe("1.0.0-beta.14");
    ...
    expect(installedCore.version).toBe("1.0.0-beta.14");
    expect(installedCore.bin["sync-engine"]).toBe("./dist/command/main.js");
    expect(installedHttp.version).toBe("1.0.0-beta.14");
