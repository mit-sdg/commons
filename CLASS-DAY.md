...
**Sorting.** The `Sorting` panel on the dashboard holds the switch `Sort automatically`. It starts off, so the cards wait in the tray until you turn it on. On, the model places cards in piles, and the panel says `sorting…` until the wall settles. Open `Sorting instructions` to see the relay's note to the sorter beside a box for this run's note; whatever you save there reaches the model on its next ask. `Resort` empties the piles and requests sorting. With the switch off the same button reads `Move all responses to tray`, and you can sort by hand: drag a card onto a pile or onto `New pile`, or open the card's `Move to` menu, and drag a card to `Remove` to take it off the wall. Turning the switch off or moving responses to the tray does not cancel a reply already in progress; it may still place cards. Wait for the current sort to finish before arranging a wall you want to keep.

**When the model is slow or gone.** The panel says `The model is not answering.` after an ask fails while `Sort automatically` is on. Turn the switch off, which takes the word with it, sort by hand, and carry on; the room sees the wall you make. When the model is back, turn the switch on and press `Resort`, or leave the wall as it stands. Without a key, asks fail; leave the switch off and sort by hand from the start, and invite no `Model seats`, since a seat with no model never hands in.

**When the dashboard says `No connection.`** The line stands above the wall once the dashboard has had no answer for ten seconds; the screen keeps trying on its own, and the line goes one poll after the server answers again. Nothing else on the laptop needs doing. The counts and the wall are as old as the line is; the wall the room saw last is real, and every student's answers are on their own phone until it sends them. `Open` and `Close` are still safe to press: each is a single request the server answers, and the button waits for that answer. Do not restart the stack, which loses the class. The projector says nothing about the connection or the model, so a TA who wants to know opens the dashboard on their own device. A student whose phone shows `No connection.` at the top waits, or presses `Retry`; the phone keeps trying either way, and what they wrote stays on it.
...
Press `Close run` on the dashboard and confirm. New joins and hand-ins are refused; requests already admitted may finish. Sorting already in progress, or a final sort requested by closing, may also finish afterward. The dashboard keeps the walls, the tallies, and the scores, and the relay's overview lists the run by when it opened and closed. Read them, or save them, before stopping the stack: press Ctrl-C once in the terminal, and the temporary database goes with it.
