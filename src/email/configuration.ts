...
export function mailConfigurationFromEnv(
  ...
  const host = configured(env.SMTP_HOST);
  const from = configured(env.SMTP_FROM);
  ...
  const sourcePort = configured(env.SMTP_PORT) ?? "587";
  ...
  const user = configured(env.SMTP_USERNAME);
  const password = configured(env.SMTP_PASSWORD);
