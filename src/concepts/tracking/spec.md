...
```actions
register (item: Item, scope: Scope) : return (item: Item), refuse (message: String)
  ...
  then
    refuse "This item is already being tracked."
...
markSeen (user: User, item: Item) : return (item: Item), refuse (message: String)
  ...
  where item not in registered
  then
    refuse "This item is not being tracked."
  where some seen-mark has this user and item
  then
    refuse "This user has already seen this item."
