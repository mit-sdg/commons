...
```actions
defineRole (name: String, capabilities: set of Strings) : return (role: Role), refuse (message: String)
  ...
  then
    refuse "A role with this name already exists."

ensureRole (name: String, capabilities: set of Strings) : return (role: Role)
  where some role has name name
  then
    return that role
  where no role has name name
...
grant (user: User, context: Context, role: Role) : return (grant: Grant), refuse (message: String)
  ...
  then
    refuse "No such role exists."
  where some grant has user, context, and role
  then
    refuse "The user already holds this role in this context."

revoke (user: User, context: Context, role: Role) : return (grant: Grant), refuse (message: String)
  where some grant has user, context, and role
  then
    delete that grant
    return grant
  ...
  then
    refuse "The user does not hold this role in this context."

requireCapability (user: User, context: Context, capability: String) : return (allowed: Boolean), refuse (message: String)
  where the user holds a granted role in the context that includes capability
  then
    return allowed true
  otherwise
    refuse "The user does not hold the required capability in this context."
```
