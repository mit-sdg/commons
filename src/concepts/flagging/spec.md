...
```actions
flag (reporter: User, target: Target, reason: String, at: Date) : return (flag: Flag), refuse (message: String)
  ...
  then
    refuse "You already have an open flag on this."

resolve (target: Target, outcome: String) : return (), refuse (message: String)
  where outcome is neither "upheld" nor "dismissed"
  then
    refuse "An outcome must be upheld or dismissed."
  ...
  then
    refuse "There are no open flags on this."
