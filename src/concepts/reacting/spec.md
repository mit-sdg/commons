...
```actions
react (reactor: Person, target: Target, kind: String, at: Date) : return (reaction: Reaction), refuse (message: String)
  ...
  then
    refuse "This person has already reacted to the target with this kind."

unreact (reactor: Person, target: Target, kind: String) : return (reaction: Reaction), refuse (message: String)
  where some reaction has this reactor, target, and kind
  then
  ...
  where no reaction has this reactor, target, and kind
  then
    refuse "There is no such reaction to take back."
