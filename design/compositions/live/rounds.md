...
When the round's presentation is captured at open, [Live.rounds.CapturedRoundSeedsStandingPiles](reaction:Live.rounds.CapturedRoundSeedsStandingPiles) ensures each standing pile on the round's wall with its name. [Live.rounds.SeededPileKeepsDefinition](reaction:Live.rounds.SeededPileKeepsDefinition) captures nonempty definitions separately in Guiding under `pile-definition`, so the wall opens with the piles already standing, empty, and the sorter's passage lists them like any pile. The relay's read carries each round's piles and its note, so the editor shows them beside the question.
