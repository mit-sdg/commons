...
[Forum.profiles.GetProfileDisplays](reaction:Forum.profiles.GetProfileDisplays) accepts at most 64 account identifiers and returns only display names, avatars, and the name of any course role held through [the display-identity former](former:Forum.profiles.theProfileDisplays). The role name comes from [the role-name former](former:Access.roles.theRoleNameOf) so a forum can mark staff beside their names in one batched read; a person with no role carries a `null` name, and the role's capabilities are never included. [profileDisplayReader](view:Forum.profiles.profileDisplayReader) admits the caller's own identity, or other identities when the caller is an active course member or holds `course:manage`. Missing and inaccessible identities are omitted; duplicate identifiers appear once. The bounded read uses Profiling's existing batch query and exposes neither biographies nor email addresses, including to staff.
